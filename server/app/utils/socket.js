'use strict';
const user = require('../models/user');
const message = require('../models/message');
class Socket{
	constructor(socket){
        this.io = socket;
    }
    
	socketEvents(){
		this.io.on('connection', (socket) => {
			/**
			* get the user's Chat list
			*/
			socket.on('chat-list', (data) => {

				let chatListResponse = {};

				if (data.userId == '') {

					chatListResponse.error = true;
					chatListResponse.message = `User does not exits.`;
					this.io.emit('chat-list-response',chatListResponse);

				}else{
                    user.findById(data.userId, (err, UserInfoResponse)=>{
						delete UserInfoResponse.password;

						user.getAll((err, response)=> {
							this.io.to(socket.id).emit('chat-list-response',{
								error : false ,
								singleUser : false ,
								chatList : response
							});

							socket.broadcast.emit('chat-list-response',{
								error : false ,
								singleUser : true ,
								chatList : UserInfoResponse
							});
						});
						// message.findOne( socket.id,(err, response)=>{
						
						// 	this.io.to(socket.id).emit('chat-list-response',{
						// 		error : false ,
						// 		singleUser : false ,
						// 		chatList : response
						// 	});

						// 	socket.broadcast.emit('chat-list-response',{
						// 		error : false ,
						// 		singleUser : true ,
						// 		chatList : UserInfoResponse
						// 	});

						// });
					});
				}
		    });

			/**
			* send the messages to the user
			*/
			socket.on('add-message', (data) => {
				
				if (data.message === '') {
					
					this.io.to(socket.id).emit(`add-message-response`,`Message cant be empty`); 

				}else if(data.fromUserId === ''){
					
					this.io.to(socket.id).emit(`add-message-response`,`Unexpected error, Login again.`); 

				}else if(data.toUserId === ''){
					
					this.io.to(socket.id).emit(`add-message-response`,`Select a user to chat.`); 

				}else{
                    let messageInfo = {
                        toUserId: data.toUserId,
                        fromUserId: data.fromUserId,
                        date: data.date,
                        message: data.message,
					}
                        // delete data.toSocketId;

					message.create(messageInfo,( error , response)=>{
						this.io.to(data.toSocketId).emit(`add-message-response`, messageInfo); 
					});
				}				
		    });


			/**
			* Logout the user
			*/
			socket.on('logout',(data)=>{

				const userId = data.userId;
				
				user.logout(userId, (error, result)=>{

					this.io.to(socket.id).emit('logout-response',{
						error : false
					});

					socket.broadcast.emit('chat-list-response',{
						error : false ,
						userDisconnected : true ,
						socketId : socket.id
					});
				});	
		    });
			/**
			* sending the disconnected user to all socket users. 
			*/
			socket.on('disconnect',()=>{
				socket.broadcast.emit('chat-list-response',{
					error : false ,
					userDisconnected : true ,
					socketId : socket.id
				});
		    });

		});

	}
	
	socketConfig(){

		this.io.use(function(socket, next) {
          	const data = {
                userId : socket.request._query['userId'],
                socketId : socket.id,
			  }
          	user.addSocketId( data ,(error,response)=>{
          		// socket id updated.
          	});
          	next();
        });

        this.socketEvents();
	}
}
module.exports = Socket;